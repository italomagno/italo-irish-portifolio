        ...
        <div className="lg:flex space-y-8 lg:space-y-0">
            ...
            <div className="w-full lg:w-1/3 lg:order-1 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-1 gap-6 lg:gap-8">
                <div>
                    ...
                    <p className="text-white/70 leading-[1.75]">{aboutData.mainData.biography}</p>
                </div>
                <div>
                    ...
                    <ul className="text-white/70">
                        {aboutData.skills.map((item, index) => (
                            <li
                                key={index}
                                className={index === 0 ? "list-none inline-block pr-[4px]" : "list-none inline-block relative pl-[14px] pr-[4px] before:content-[''] before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:w-[5px] before:h-[5px] before:rounded-md before:bg-white/80"}
                            >
                                {item.name}
                            </li>
                        ))}
            ...
            <div className="w-full lg:w-1/3 order-3 grid grid-cols-3 lg:grid-cols-1 gap-6 lg:gap-7 lg:text-right">
                ...
                <div>
                    <h6 className="font-outfit font-medium tracking-wider uppercase text-sm text-white mb-2">Worldwide Clients</h6>
                    <span className="text-4xl lg:text-5xl xl:text-6xl font-outfit font-light text-white">{aboutData.mainData.worldwideClients}</span>
                </div>
            </div>
        </div>
    )
